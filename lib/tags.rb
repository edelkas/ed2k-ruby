module ED2K
  ...
  module Tag
    ...
    # @param data [String,IO] The raw binary data to unpack the tags from. Note that if you pass an IO object, the stream
    #                         will be advanced to the end of the taglist. Strings will not be mutated.
    ...
    def self.read(data, offset = 0, core: nil)
      stream = data.is_a?(String) ? StringIO.new(data) : data
