...
/* These were originally in a separate header file */
...
/* Compute the MD4 hash of a raw block of data in a single call */
void md4_data(unsigned char* out, unsigned char* in, size_t size) {
	MD4_CTX ctx;
	MD4_Init(&ctx);
    MD4_Update(&ctx, in, size);
	MD4_Final(out, &ctx);
}

/* Compute the MD4 hash of a file, read in 4KB increments */
